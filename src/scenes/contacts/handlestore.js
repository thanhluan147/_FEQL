import Axios from "axios";
import Url_BackEnd from "../../URL";
export const Get_all_store_By_userid = async (req) => {
  const respod = await Axios.get(
    `${Url_BackEnd}/store/getstoreByUserID`,

    {
      headers: {
        "Content-Type": "application/json",
        // Thêm các header khác nếu cần
        Authorization: `Bearer ${localStorage.getItem("token")}`,
      },
    }
  );

  return JSON.stringify(respod.data.All_Branch);
};

export const Get_all_Store = async (req) => {
  const respod = await Axios.get(`${Url_BackEnd}/store/getallstore`, {
    headers: {
      "Content-Type": "application/json",
      // Thêm các header khác nếu cần
      Authorization: `Bearer ${localStorage.getItem("token")}`,
    },
  });
  return JSON.stringify(respod.data.All_Store);
};

export const Get_all_User_By_branchID = async (req) => {
  const respod = await Axios.post(
    `${Url_BackEnd}/Staff/getallStaft/`,
    {
      branchID: req,
    },
    {
      headers: {
        "Content-Type": "application/json",
        // Thêm các header khác nếu cần
        Authorization: `Bearer ${localStorage.getItem("token")}`,
      },
    }
  );

  return JSON.stringify(respod.data.All_Branch);
};
